// ============================================
// INYECCIÓN DINÁMICA DE PRESUPUESTOS MODULE
// ============================================
// Este código se ejecuta INMEDIATAMENTE cuando se carga el script
// Usa múltiples estrategias para asegurar que se ejecute

function inyectarPresupuestos() {
  console.log('🔄 Intentando inyectar Presupuestos...')
  
  // 1. Inyectar botón de navegación de Presupuestos
  const clientesButton = document.querySelector('button[onclick="showTab(\'clientes\')"]')
  if (clientesButton && !document.querySelector('button[onclick="showTab(\'presupuestos\')"]')) {
    const presupuestosButton = document.createElement('button')
    presupuestosButton.setAttribute('onclick', 'showTab(\'presupuestos\')')
    presupuestosButton.className = 'tab-button px-6 py-3 rounded-lg font-medium transition-all text-gray-700 hover:bg-gray-100'
    presupuestosButton.innerHTML = '<i class="fas fa-file-alt mr-2"></i>Presupuestos'
    clientesButton.parentNode.insertBefore(presupuestosButton, clientesButton.nextSibling)
    console.log('✅ Botón Presupuestos inyectado')
  } else {
    console.warn('⚠️ No se pudo inyectar botón (clientesButton no encontrado o ya existe)')
  }
  
  // 2. Inyectar tab content de Presupuestos
  const clientesTab = document.getElementById('clientes-tab')
  if (clientesTab && !document.getElementById('presupuestos-tab')) {
    const presupuestosTab = document.createElement('div')
    presupuestosTab.id = 'presupuestos-tab'
    presupuestosTab.className = 'tab-content'
    presupuestosTab.innerHTML = `
    ...
    `
    clientesTab.parentNode.insertBefore(presupuestosTab, clientesTab.nextSibling)
    console.log('✅ Tab Presupuestos inyectado')
  } else {
    console.warn('⚠️ No se pudo inyectar tab (clientesTab no encontrado o ya existe)')
  }
}

// Estrategia 1: DOMContentLoaded
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', inyectarPresupuestos)
} else {
  // Estrategia 2: DOM ya está listo, ejecutar inmediatamente
  inyectarPresupuestos()
}

// Estrategia 3: Reintentar después de 500ms por si acaso
setTimeout(inyectarPresupuestos, 500)

// Estrategia 4: Reintentar después de 1000ms como último recurso
setTimeout(inyectarPresupuestos, 1000)
